...
export default function ModalFormPublicInformation({ isOpen, closeModal, action, item, infoId }: ModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      if (action === "create") return await apiV1.post("/public-information", data, { headers: { "Content-Type": "multipart/form-data" } });
      if (action === "update") return await apiV1.put(`/public-information/${infoId}`, data, { headers: { "Content-Type": "multipart/form-data" } });
      if (action === "delete") return await apiV1.delete(`/public-information/${infoId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["publicInformation"] });
      closeModal();
    },
    onError: (error) => {
      console.error(error);
  ...
  return (
    <Modal isOpen={isOpen} onClose={closeModal} className="max-w-2xl p-6">
      <h3 className="text-lg font-bold mb-4">{action === "create" ? "Tambah Informasi Publik" : "Edit Informasi Publik"}</h3>
