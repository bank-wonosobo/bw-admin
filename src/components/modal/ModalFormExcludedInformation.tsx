...
export default function ModalFormExcludedInformation({ isOpen, closeModal, action, item, infoId }: ModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      if (action === "create") return await apiV1.post("/excluded-information", data, { headers: { "Content-Type": "multipart/form-data" } });
      if (action === "update") return await apiV1.put(`/excluded-information/${infoId}`, data, { headers: { "Content-Type": "multipart/form-data" } });
      if (action === "delete") return await apiV1.delete(`/excluded-information/${infoId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["excludedInformation"] });
      closeModal();
    },
    onError: (error) => {
      console.error(error);
  ...
  return (
    <Modal isOpen={isOpen} onClose={closeModal} className="max-w-2xl p-6">
      <h3 className="text-lg font-bold mb-4">{action === "create" ? "Tambah Informasi Dikecualikan" : "Edit Informasi Dikecualikan"}</h3>
