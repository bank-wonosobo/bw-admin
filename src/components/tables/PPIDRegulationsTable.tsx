...
import { IPPIDRegulations } from "@/types/PPIDRegulations";
import { useQuery } from "@tanstack/react-query";
import { useState } from "react";
import ModalFormPPIDRegulations from "../modal/ModalFormPPIDRegulations";
import {
  Table,
  TableBody,
  TableCell,
...
export default function PPIDRegulationsTable() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery<IPPIDRegulations[]>({
    queryKey: ["PPIDRegulations", currentPage],
    queryFn: async () => {
      const response = await apiV1.get("/ppid-regulations", {
  ...
  function showModal(act: any, id: any, itemList?: IPPIDRegulations[]) {
    setAction(act);
    setInfoId(id);
