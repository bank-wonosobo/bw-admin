...
import ModalFormOrganizationalStructure from "../modal/ModalFormOrganizationalStructure";
...
export default function OrganizationalStructureTable() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery<IOrganizationalStructure[]>({
    queryKey: ["organizational-structures", currentPage],
    queryFn: async () => {
      const response = await apiV1na.get("/organizational-structures", {
  ...
  const [organizationalStructureId, setOrganizationalStructureId] = useState<
    string | null
  >(null);
  const [item, setItem] = useState<IOrganizationalStructure | null>(null);

  function showModal(
    act: any,
    id: string,
    items?: IOrganizationalStructure[]
  ) {
    setAction(act);
    setOrganizationalStructureId(id);
  ...
  return (
    <>
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <ModalFormOrganizationalStructure
          isOpen={isOpen}
          action={action}
